...
^ The _closing tag_ consists of a left angle bracket, a foward slash, the element name and the right angled bracket.
...
^ Not long ago, HTML documents were coded so the HTML not only defined the content and structure of the web page but also the formatting of that content. The mix of structural and formatting elements made it hard to edit, maintain and reformat the web pages.
...
^ We're going to learn how to code CSS, but here's a brief introduction to how the CSS works. CSS files consist of _rule sets_. Each of these rule sets consists of a _selector_ and a _declaration block_. The selector indentifies an HTML element, and the declaration block specifies the formatting for the element. Within the declaration block are one or more _declarations_ (or rules) that specify the formatting for the element.
