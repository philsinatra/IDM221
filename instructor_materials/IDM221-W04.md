...
^ A problem web developers face today when it comes to CSS is that the five modern browsers render some elements of a web page differently. One example is the `abbr` tag, which in some browsers has a dotted underline, where as other browsers do not include the underline, making it hard for users to know the abbreviation is there.
...
^ To standardize how elements like this are displayed, you can use the _normalize.css_ style sheet. This style sheet applies styles to elements like the `abbr` element so they appear exactly the same in all browsers. This can save you a lot of time dealing with small rendering issues near the end of a project.
