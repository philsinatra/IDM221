...
^ How do users use a page? They don't read it in an orderly fashion, and they don't like to scroll any more than they have to. If they can't find what they're looking for or get frustrated, they leave the site.

^ In web development terms, what users want is _usability_. The term refers to how easy it is to use a website. If a site is easy to use, it has a chance to be effective.

^ present critical content above the fold
^ group related items and limit the number of groups
^ include a header that identifies the site and provides navigation
^ use current navigation conventions (e.g. logo goes to home page)
...
- [Mac](https://github.com/github/gitignore/blob/master/Global/OSX.gitignore)
