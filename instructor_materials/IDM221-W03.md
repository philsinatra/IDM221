...
^ By default, links are underline when they are displayed in a browser to indicate that they are clickable. When a link is displayed, it has a default color depending on its state. Four instance, link that has not been visited is displayed in bloom, and a link that has been visited is displayed in purple. Later we'll use CSS to change these settings.

^ When you create a link that contains text, the text should clearly indicate the function of the link. For example, you should not use text like "click here" because it does not indicate what the link does. Instead, you should use text like that in the examples. If you can't tell what a link the house by reading it's text, you should rewrite the text. This improves the accessibility of your site, and help search engines index your site.
